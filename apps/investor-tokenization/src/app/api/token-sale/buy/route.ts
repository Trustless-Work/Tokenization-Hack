...
export async function POST(request: Request) {
    ...
    if (!tokenSaleContractId || !usdcAddress || !payerAddress || !beneficiaryAddress) {
        return new Response(
            JSON.stringify({
                error: "Missing required fields",
                details: "tokenSaleContractId, usdcAddress, payerAddress, and beneficiaryAddress are required",
            }),
            { status: 400 }
        );
    }
    ...
    try {
        ...
        return NextResponse.json({
            success: true,
            xdr,
            message: "Transaction built successfully. Sign with wallet and submit to network.",
        });
    } catch (error) {
